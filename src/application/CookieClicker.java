...
public class CookieClicker extends Application{
	...
	private static File folder = new File(System.getenv("APPDATA") + "\\cookieclicker");
	private static File file = new File (System.getenv("APPDATA") + "\\cookieclicker\\save.txt");
	
	private static String folderS = (System.getenv("APPDATA") + "\\cookieclicker");
	private static String fileS = (System.getenv("APPDATA") + "\\cookieclicker\\save.txt");
	...
	public static void saveToFile(MainController mainController){
		...
		try {
			...
			System.err.println("///File could not be saved in " + file + " Error: " + e);
		}
	}
	...
	public static void readFromFile(MainController mainController) {
		...
		sc = new Scanner(new File (fileS));
			...
			System.err.println("///File in " + file + " could not be read: " + e);
		}
	...
	static void resetSave(MainController mainController) {
		try {
			...
			System.err.println("///File in " + file + " could not be wiped: " + e);
		}
	}	
	...
	static void checkForFile() {
		if(folder.exists()) {
			...
			if(file.exists()) { 
				...
				System.err.println("///File could not be found in: " + folder);
				resetSave(mainController);
				System.out.println("New savefile created:" + file);	
			}
		}else {
			System.err.println("///Folder could not be found in: " + folder);
